...
func TestGitFileNodeInsertAndGet(t *testing.T) {
	root := &gitFileNode{name: "."}
	root.insert("a/b/c.txt", "hash-1")
	root.insert("a/d.txt", "hash-2")

	node, ok := root.get("a/b/c.txt")
	if !ok || node.hash != "hash-1" || node.isDir {
		t.Fatalf("get(a/b/c.txt) = %+v, %v", node, ok)
	}
	node, ok = root.get("a")
	if !ok || !node.isDir {
		t.Fatalf("get(a) = %+v, %v", node, ok)
	}
	if _, ok := root.get("missing.txt"); ok {
		t.Fatal("missing path unexpectedly found")
	}
}
